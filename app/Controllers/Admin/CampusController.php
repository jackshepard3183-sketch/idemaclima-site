...
final class CampusController
{
    ...
    public static function saveEvent(): void
    {
        ...
        $audience=in_array($_POST['audience']??'', ['public','cat'],true)?$_POST['audience']:'public';$starts=self::mysqlDateTime($_POST['starts_at']??null);if($starts===null)$errors[]='Data inizio obbligatoria o non valida.';
        $ends=self::mysqlDateTime($_POST['ends_at']??null);$deadline=self::mysqlDateTime($_POST['registration_deadline']??null);
        $data=['id'=>$id,'title'=>$title,'slug'=>$slug,'audience'=>$audience,'location'=>trim((string)($_POST['location']??'')),'address'=>trim((string)($_POST['address']??'')),'starts_at'=>$starts??'','ends_at'=>$ends??'','short_description'=>trim((string)($_POST['short_description']??'')),'description'=>trim((string)($_POST['description']??'')),'max_seats'=>Validator::int($_POST['max_seats']??0)?:null,'registration_open'=>Validator::bool($_POST['registration_open']??0),'registration_deadline'=>$deadline,'published'=>Validator::bool($_POST['published']??0),'cancelled'=>Validator::bool($_POST['cancelled']??0),'sort_order'=>Validator::int($_POST['sort_order']??0)];
        if($errors){self::view('campus_event_form',['title'=>'Evento Campus','event'=>$data,'errors'=>$errors]);return;}
        $pdo=Database::connection();
        try {
            if($id){$s=$pdo->prepare('UPDATE events SET title=?,slug=?,audience=?,location=?,address=?,starts_at=?,ends_at=?,short_description=?,description=?,max_seats=?,registration_open=?,registration_deadline=?,published=?,cancelled=?,sort_order=? WHERE id=?');$s->execute([$title,$slug,$audience,$data['location']?:null,$data['address']?:null,$starts,$ends,$data['short_description']?:null,$data['description']?:null,$data['max_seats'],$data['registration_open'],$deadline,$data['published'],$data['cancelled'],$data['sort_order'],$id]);}
            else{$s=$pdo->prepare('INSERT INTO events(title,slug,audience,location,address,starts_at,ends_at,short_description,description,max_seats,registration_open,registration_deadline,published,cancelled,sort_order) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');$s->execute([$title,$slug,$audience,$data['location']?:null,$data['address']?:null,$starts,$ends,$data['short_description']?:null,$data['description']?:null,$data['max_seats'],$data['registration_open'],$deadline,$data['published'],$data['cancelled'],$data['sort_order']]);}
        } catch (\PDOException $e) {
            if ((string)$e->getCode()==='23000') { $errors[]='Slug già utilizzato da un altro evento.'; self::view('campus_event_form',['title'=>'Evento Campus','event'=>$data,'errors'=>$errors]); return; }
            throw $e;
        }
    ...
    private static function mysqlDateTime(mixed $value):?string{$v=trim((string)$value);if($v==='')return null;$ts=strtotime($v);return $ts===false?null:date('Y-m-d H:i:s',$ts);}
}
