...
final class ContentController
{
    ...
    public static function saveAlbum(): void
    {
        ...
        header('Location:/admin/content/gallery/form?id='.$id);exit;
    }
    ...
    public static function archiveAlbum(): void
    {
        ...
        Audit::log('gallery.album.archive','gallery_album',$id,[]);header('Location:/admin/content/gallery');exit;
    }
    ...
    public static function saveReference(): void
    {
        ...
        header('Location:/admin/content/references/form?id='.$id);exit;
    }

    public static function archiveReference(): void
    {
        AdminAuth::requireLogin();self::csrf();$id=Validator::int($_POST['id']??0);$pdo=Database::connection();$s=$pdo->prepare('UPDATE references_projects SET archived_at=NOW(),published=0 WHERE id=? AND archived_at IS NULL');$s->execute([$id]);if($s->rowCount()!==1)self::notFound('Referenza non trovata');Audit::log('reference.archive','reference_project',$id,[]);header('Location:/admin/content/references');exit;
    }
    ...
    public static function duplicateReference(): void
    {
        ...
        header('Location:/admin/content/references/form?id='.$newId);exit;
    }
    ...
    private static function redirectAlbum(int $id):never{header('Location:/admin/content/gallery/form?id='.$id);exit;}
    private static function redirectReference(int $id):never{header('Location:/admin/content/references/form?id='.$id);exit;}
