...
final class MediaLibraryController
{
    ...
    {
        AdminAuth::requireLogin();self::csrf();$id=Validator::int($_POST['id']??0);$pdo=Database::connection();$s=$pdo->prepare('SELECT file_path FROM media_assets WHERE id=? AND archived_at IS NULL');$s->execute([$id]);$path=$s->fetchColumn();if(!is_string($path))self::redirect('','File non trovato.');$uses=self::usageCount($pdo,$path);if($uses>0)self::redirect('','Il file è usato in '.$uses.' contenuti: rimuovi prima i collegamenti.');$pdo->prepare('UPDATE media_assets SET archived_at=NOW() WHERE id=?')->execute([$id]);Audit::log('media.archive','media_asset',$id,['path'=>$path]);self::redirect('File archiviato.');
    }
    ...
    private static function syncExisting():void
    {
        ...
        foreach($it as $file){if(!$file->isFile()||$seen>=5000)continue;$relative=str_replace('\\','/',substr($file->getPathname(),strlen(dirname(__DIR__,3).'/public')));$mime=(string)(new \finfo(FILEINFO_MIME_TYPE))->file($file->getPathname());if(!isset(self::IMAGE_MIMES[$mime])&&!isset(self::PDF_MIMES[$mime]))continue;$category=isset(self::PDF_MIMES[$mime])?'documents':'images';$title=trim(str_replace(['-','_'],' ',pathinfo($file->getFilename(),PATHINFO_FILENAME)));$insert->execute([$title?:$file->getFilename(),$file->getFilename(),$relative,$mime,$file->getSize(),hash_file('sha256',$file->getPathname()),$category]);$seen++;}
    }
    ...
    private static function usageCount(PDO $pdo,string $path):int{$queries=[['products','image_path'],['documents','file_path'],['catalogs','cover_image'],['catalogs','pdf_path'],['gallery_albums','cover_image'],['gallery_images','image_path'],['references_projects','cover_image'],['reference_images','image_path']];$n=0;foreach($queries as [$table,$column]){try{$s=$pdo->prepare("SELECT COUNT(*) FROM {$table} WHERE {$column}=?");$s->execute([$path]);$n+=(int)$s->fetchColumn();}catch(\Throwable){}}return $n;}
