...
final class AssistanceController
{
    public static function index(): void
    {
        ...
        $stmt = $pdo->query(
            'SELECT ar.id,ar.section,ar.label,ar.document_id,ar.external_url,d.published document_published
             ...
             WHERE ar.published=1
               ...
               AND (ar.document_id IS NULL OR d.published=1)
        ...
        $configured = trim((string)(getenv('ASSISTANCE_FORM_URL') ?: ''));
        $supportFormUrl = filter_var($configured, FILTER_VALIDATE_URL) ? $configured : 'https://idemaassistenza.it/';
        $scheme = strtolower((string)(parse_url($supportFormUrl, PHP_URL_SCHEME) ?: ''));
        if (!in_array($scheme, ['http','https'], true)) {
            $supportFormUrl = 'https://idemaassistenza.it/';
        }
