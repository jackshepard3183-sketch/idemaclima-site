...
final class ContentController
{
    ...
    {
        $rows=Database::connection()->query('SELECT a.*,COUNT(i.id) image_count FROM gallery_albums a LEFT JOIN gallery_images i ON i.album_id=a.id AND i.published=1 WHERE a.published=1 GROUP BY a.id ORDER BY a.sort_order,a.title')->fetchAll(PDO::FETCH_ASSOC);
        self::render('content/gallery',['title'=>'Galleria','albums'=>$rows]);
    }

    public static function galleryAlbum(string $slug): void
    {
        $pdo=Database::connection();$s=$pdo->prepare('SELECT * FROM gallery_albums WHERE slug=? AND published=1 LIMIT 1');$s->execute([$slug]);$album=$s->fetch(PDO::FETCH_ASSOC);
    ...
    {
        $rows=Database::connection()->query('SELECT * FROM references_projects WHERE published=1 ORDER BY sort_order,title')->fetchAll(PDO::FETCH_ASSOC);
        self::render('content/references',['title'=>'Referenze','references'=>$rows]);
    }

    public static function reference(string $slug): void
    {
        $pdo=Database::connection();$s=$pdo->prepare('SELECT * FROM references_projects WHERE slug=? AND published=1 LIMIT 1');$s->execute([$slug]);$reference=$s->fetch(PDO::FETCH_ASSOC);
