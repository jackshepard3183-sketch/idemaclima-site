...
final class Upload
{
    ...
    {
        return self::store($field, 'products', self::IMAGE_MAX_BYTES, [
            'image/jpeg' => 'jpg',
            'image/png' => 'png',
            'image/webp' => 'webp',
        ], true, $errors);
    ...
    {
        return self::store($field, 'documents', self::PDF_MAX_BYTES, [
            'application/pdf' => 'pdf',
        ], false, $errors);
    }
    ...
    private static function store(
        ...
        if ($error !== UPLOAD_ERR_OK) {
            $errors[] = 'Caricamento file non riuscito (codice ' . $error . ').';
            return null;
        }
        ...
        if ($size > $maxBytes) {
            $errors[] = sprintf('File troppo grande. Dimensione massima: %d MB.', (int)($maxBytes / 1024 / 1024));
            return null;
        }
        ...
        if ($mustBeImage) {
            ...
            if ($signature !== '%PDF-') {
                $errors[] = 'Il file non è un PDF valido.';
                return null;
            }
        }

        $base = pathinfo($original, PATHINFO_FILENAME);
        $base = iconv('UTF-8', 'ASCII//TRANSLIT//IGNORE', $base) ?: 'file';
