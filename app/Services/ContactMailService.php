...
{
    public static function notify(array $request,?string $attachmentName=null):void
    {
        ...
        $subject='Nuova richiesta contatti: '.self::clean((string)($request['subject']??''));$encoded=function_exists('mb_encode_mimeheader')?mb_encode_mimeheader($subject,'UTF-8'):$subject;
        foreach($recipients as $recipient)if(!@mail($recipient,$encoded,$body,implode("\r\n",$headers)))error_log('IDEMA contact email not sent');
    }
