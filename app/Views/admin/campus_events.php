...
<div class="toolbar">
  ...
  <a class="btnlink" href="/admin/campus/events/form">Nuovo evento</a>
</div>
<div class="quick-list" style="grid-template-columns:repeat(3,minmax(0,180px));margin-bottom:18px">
  <a href="/admin/campus/events"><span>Tutti</span></a>
  <a href="/admin/campus/events?audience=public"><span>Eventi aperti</span></a>
  <a href="/admin/campus/events?audience=cat"><span>Eventi CAT</span></a>
</div>
...
<?php foreach($events as $event): ?>
  <tr>
    ...
    <td><a href="/admin/campus/registrations?event_id=<?=(int)$event['id']?>"><?=(int)$event['registrations']?></a></td>
    ...
    <td><a href="/admin/campus/events/form?id=<?=(int)$event['id']?>">Modifica</a><form method="post" action="/admin/campus/events/duplicate" style="display:inline;margin-left:8px"><input type="hidden" name="_csrf" value="<?=htmlspecialchars($csrf,ENT_QUOTES,'UTF-8')?>"><input type="hidden" name="id" value="<?=(int)$event['id']?>"><button type="submit" class="link-button">Duplica</button></form></td>
  </tr>
<?php endforeach; ?>
