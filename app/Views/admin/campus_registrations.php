...
<?php $confirmationSubject='Conferma iscrizione - '.(string)$row['event_title'];$confirmationBody="Buongiorno ".(string)$row['first_name']." ".(string)$row['last_name'].",\n\nconfermiamo la sua iscrizione all'evento \"".(string)$row['event_title']."\" del ".date('d/m/Y H:i',strtotime((string)$row['event_starts_at'])).(!empty($row['event_location'])?" presso ".(string)$row['event_location']:'').".\n\nCordiali saluti\nIdema Clima S.r.l.";$confirmationMailto='mailto:'.(string)$row['email'].'?subject='.rawurlencode($confirmationSubject).'&body='.rawurlencode($confirmationBody); ?>
  <tr>
    ...
    <td data-label="Contatti"><?=htmlspecialchars($row['email'],ENT_QUOTES,'UTF-8')?><?=!empty($row['phone'])?'<br>'.htmlspecialchars($row['phone'],ENT_QUOTES,'UTF-8'):''?><?php if(!empty($row['confirmation_sent_at'])):?><br><span style="color:#3f7d0b;font-weight:700">✓ Conferma inviata<br><?=htmlspecialchars(date('d/m/Y H:i',strtotime((string)$row['confirmation_sent_at'])),ENT_QUOTES,'UTF-8')?></span><?php endif;?></td>
