...
<?php require __DIR__.'/_layout_end.php'; ?><?php require __DIR__.'/_layout_start.php'; ?>
...
<?php if(!empty($replySentAt)):?><p style="color:#3f7d0b;font-weight:700">✓ Email segnata come inviata il <?= htmlspecialchars(date('d/m/Y H:i',strtotime((string)$replySentAt)),ENT_QUOTES,'UTF-8') ?></p><?php endif;?>
<form method="post" action="/idemaclima/admin/contacts/reply-sent"><input type="hidden" name="_csrf" value="<?= htmlspecialchars($csrf,ENT_QUOTES,'UTF-8') ?>"><input type="hidden" name="id" value="<?= (int)$row['id'] ?>"><button class="btn" type="submit"><?=!empty($replySentAt)?'Registra un nuovo invio':'Segna email come inviata'?></button></form>
