...
<div class="import-stat"><span>Garanzie importate</span><strong id="items-imported">0</strong></div>
...
const setProgress=()=>{
 const done=queue.filter(item=>item.status==='success').length;
 const percent=queue.length?Math.round(done/queue.length*100):0;
 complete.textContent=done+'/'+queue.length; imported.textContent=totals.imported; skipped.textContent=totals.skipped; errors.textContent=totals.errors;
 bar.style.width=percent+'%';track.setAttribute('aria-valuenow',String(percent));
};
const render=()=>{
 body.textContent='';
 ...
 queue.forEach((item,index)=>{
  ...
  const controls=document.createElement('span');controls.className='queue-order';
  [['↑',-1,'Sposta prima'],['↓',1,'Sposta dopo']].forEach(spec=>{const button=document.createElement('button');button.type='button';button.textContent=spec[0];button.setAttribute('aria-label',spec[2]+' '+item.file.name);button.disabled=running||(spec[1]<0?index===0:index===queue.length-1);button.addEventListener('click',()=>{const other=index+spec[1];[queue[index],queue[other]]=[queue[other],queue[index]];render()});controls.appendChild(button)});order.appendChild(controls);
  const labels={pending:'In attesa',running:'Importazione…',success:'Completato',error:'Errore'};state.className='state-'+item.status;state.textContent=labels[item.status]||item.status;if(item.detail){const d=document.createElement('span');d.className='queue-detail';d.textContent=item.detail;state.appendChild(d);}
  row.append(number,fileCell,order,state);body.appendChild(row);
 });
 start.disabled=running||!queue.length||current>=queue.length;sortButton.disabled=running||queue.length<2;input.disabled=running;
 note.textContent=queue.length?queue.length+' file selezionati.':'Nessun file selezionato.';
 setProgress();
};
...
const importOne=async item=>{
 const data=new FormData();data.append('_csrf',csrf);data.append('manifest',item.file,item.file.name);
 const response=await fetch('/idemaclima/admin/warranties/import/run',{method:'POST',body:data,credentials:'same-origin',headers:{Accept:'application/json'}});
 const result=await parseResponse(response);
 if(!response.ok||!Array.isArray(result.errors))throw new Error((result.errors||['Risposta di importazione non valida']).join(' | '));
 return result;
};
const run=async()=>{
 if(running||!queue.length)return;
 running=true;start.textContent='Importazione in corso…';log.classList.remove('is-visible');log.textContent='';render();
