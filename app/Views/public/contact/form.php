...
<label>Profilo *<select name="profile" required><option value="">Seleziona profilo</option><?php foreach(['Installatore','Progettista','Centro assistenza tecnica','Cliente privato','Altro'] as $profileOption): ?><option value="<?= e($profileOption) ?>" <?= (($old['profile']??'')===$profileOption)?'selected':'' ?>><?= e($profileOption) ?></option><?php endforeach; ?></select></label>
