(function(){
  ...
  document.querySelectorAll('.product-item:not([data-special])').forEach(function(i){i.addEventListener('toggle',function(){if(!i.open||i.dataset.loaded)return;i.dataset.loaded='1';var p=i.querySelector('.product-panel');p.innerHTML='<div class="panel-loading">Caricamento documenti…</div>';fetch(i.dataset.url,{credentials:'same-origin'}).then(function(r){return r.text()}).then(function(h){var d=new DOMParser().parseFromString(h,'text/html'),x=d.querySelector('.product-description'),g=normalizeGroups(d.querySelector('.document-groups'));p.innerHTML='<div class="inline-docs">'+(x?x.outerHTML:'')+(g?g.outerHTML:'<p>Documentazione su richiesta.</p>')+'</div>'}).catch(function(){p.innerHTML='<div class="panel-loading">Impossibile caricare i documenti. Riprova.</div>'})})});
})();