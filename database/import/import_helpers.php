...
function classifyDocumentType(string $group, string $label): string
{
    ...
    if (str_contains($g, 'MANUAL')) {
        ...
        if (preg_match('/\bUSO\b/u', $l)) return 'Manuale uso';
        if (str_contains($l, 'TELECOMANDO')) return 'Manuale telecomando';
...
function shouldCreateModel(string $label, string $productName, string $group): bool
{
    ...
    if (stripos($group, 'MANUAL') !== false) return false;
...
function isGenericDocumentLabel(string $label): bool
{
    ...
    foreach (['INSTALLAZIONE', 'USO', 'TELECOMANDO', 'MODULO WI-FI', 'MODULO WIFI', 'WI-FI', 'WIFI', 'MANUALE'] as $generic) {
