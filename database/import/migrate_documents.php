...
{
    if (strlen($bytes) < 5 || substr($bytes, 0, 5) !== '%PDF-') throw new RuntimeException('Il contenuto ricevuto non è un PDF valido.');
    if (strlen($bytes) > 50 * 1024 * 1024) throw new RuntimeException('PDF oltre il limite di 50 MB.');
}
...
$stats = ['manifest'=>count($rows),'downloaded'=>0,'verified'=>0,'physical_deduped'=>0,'db_inserted'=>0,'db_deduped'=>0,'aliases'=>0,'errors'=>0];
$report = [];
    ...
    try {
        ...
        if ($download) {
            ...
            if (!is_file($absolutePath)) {
                ...
                if ($samePhysical !== null) {
                    ...
                    if (!is_dir($targetRoot) && !mkdir($targetRoot, 0755, true) && !is_dir($targetRoot)) throw new RuntimeException('Impossibile creare la cartella documenti migrati.');
        ...
        if ($execute && $pdo instanceof PDO) {
            ...
            if ($existingRow) {
                ...
                $ins = $pdo->prepare('INSERT INTO documents(document_type_id,title,filename,file_path,sha256,file_size,mime_type,published,sort_order) VALUES(?,?,?,?,?,?,"application/pdf",1,0)');
                $ins->execute([(int)$typeId,$row['title'],$filename,$relativePath,$sha256,$size]);
                $documentId = (int)$pdo->lastInsertId();
                $stats['db_inserted']++;
            }

            $alias = $pdo->prepare('INSERT IGNORE INTO document_source_aliases(document_id,source_url,source_kind,source_page) VALUES(?,?,"wordpress",NULL)');
            ...
            if (!empty($row['category_slug'])) {
                ...
                if ($checkLink->fetchColumn() === false) $pdo->prepare('INSERT INTO document_links(document_id,category_id) VALUES(?,?)')->execute([$documentId,(int)$categoryId]);
            }

            if ($row['type_slug'] === 'dichiarazione-ce') {
                $page = $pdo->query('SELECT id FROM editorial_pages WHERE page_key="ce" LIMIT 1');
...
if (!$download) echo "Nessun file scaricato. Usa --download per copiare/verificare/deduplicare i PDF, --execute solo per inserire anche nel DB.\n";
