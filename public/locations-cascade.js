(() => {
  ...
  const initial = {
    region: region.dataset.initial || region.dataset.selected || '',
    province: province.dataset.initial || province.dataset.selected || '',
    city: city.dataset.initial || city.dataset.selected || '',
    postalCode: postalCode.dataset.initial || postalCode.dataset.selected || ''
  };
  ...
  fetch('/idemaclima/public/locations-cascade.json', { credentials: 'same-origin' })
    .then(response => {
