...
$root=dirname(__DIR__);$family=file_get_contents($root.'/app/Views/public/technical_sheets/family.php');$product=file_get_contents($root.'/app/Views/public/technical_sheets/product.php');$controller=file_get_contents($root.'/app/Controllers/Public/TechnicalSheetsController.php');$search=file_get_contents($root.'/app/Views/public/technical_sheets/search.php');
foreach([$family,$product,$controller,$search] as $content)if(!is_string($content)||$content==='')throw new RuntimeException('File Schede tecniche non leggibile.');
